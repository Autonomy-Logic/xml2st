    ...
    def CreateClasses(self):
        ...
        for name, infos in list(self.Namespaces[self.TargetNamespace].items()):
            if infos["type"] == ELEMENT:
                if (
                    not isinstance(infos["elmt_type"], str)
        ...
        for element in classinfos["elements"]:
            if element["type"] == CHOICE:
                ...
                if element["maxOccurs"] == "unbounded" or element["maxOccurs"] > 1:
                    classmembers["append%sbytype" % elmtname] = (
                        generateAppendChoiceByTypeMethod(
                            element["maxOccurs"], self, element["choices"]
                        )
                    )
                    classmembers["insert%sbytype" % elmtname] = (
                        generateInsertChoiceByTypeMethod(
                            element["maxOccurs"], self, element["choices"]
                        )
                    )
                else:
                    classmembers["set%sbytype" % elmtname] = (
            ...
            if element_infos["type"] == CHOICE:
                ...
                if (
                    ...
                    if element_infos["elmt_type"]["type"] == SIMPLETYPE:
                        return [
                            element_infos["elmt_type"]["extract"](
                    ...
                    if element_idx > 0:
                        previous_elements_xpath = "|".join(
                            [
                                ...
                                for x in list(elements.keys())[:element_idx]
                            ]
                        )
...
def GenerateParser(factory, xsdstring):
    ...
    BaseClass = [
        (name, XSDclass)
        for name, XSDclass in list(ComputedClasses.items())
